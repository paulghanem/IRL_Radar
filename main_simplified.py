...
# IMPORTANT: Import SimplifiedWalker dynamics wrapper
from src.control.dynamics import kinematics_simplified_walker

# Monkey-patch mppi_class to use SimplifiedWalker dynamics instead of MuJoCo
...
print("\n" + "="*70)
print("USING SIMPLIFIED WALKER2D DYNAMICS FOR AGENT ROLLOUTS")
print("="*70)
print("Expert demos: Real Walker2d (PPO trained policy)")
print("Agent rollouts: SimplifiedWalker2d (via MPPI)")
print("NOTE: Physics mismatch between expert and agent!")
print("="*70 + "\n")
...
# Expert generation uses original GenerateDemo (real Walker2d PPO expert)
# No custom function needed - will use GenerateDemo class below
...
parser = argparse.ArgumentParser(description = 'GCL+MPPI with SimplifiedWalker2d', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
...
parser.add_argument('--results_savepath', default="results_simplified",type=str, help='Folder to save bigger results folder')
parser.add_argument('--experiment_name', default="experiment_simplified",type=str, help='Name of folder to save temporary images to make GIFs')
...
parser.add_argument('--gym_env', default="Walker2d",type=str, help='gym environment to test (Walker2d with SimplifiedWalker2d)')
...
assets_dir="assets"
args.dt=0.002  # Walker2d dt
args.frame_skip=4  # Walker2d frame_skip

# For SimplifiedWalker, we don't need MuJoCo model, but we create a dummy for compatibility
...
for runs in range (args.runs):
    ...
    for i in range(args.rirl_iterations):
        ...
        print("=" * 70)
        if i== 0:
            # GENERATE EXPERT DEMOS USING REAL WALKER2D (PPO trained)
            # Agent will use SimplifiedWalker2d for rollouts
            print("\nGenerating expert demos from real Walker2d (PPO policy)...")
            ...
            print("Expert demo rewards:", rewards_demo[-1] if len(rewards_demo) > 0 else 0)
            ...
            u_min = -jnp.ones(args.a_dim)
            u_max = jnp.ones(args.a_dim)
            cov_scaler = jnp.ones(args.a_dim)  # Default covariance
            ...
            policy = MPPI(
                ...
                dynamics=None,  # Not used with SimplifiedWalker
                cost_func=jax.jit(vmap(cost_function,in_axes=(0,None))),
                u_min=u_min,
                u_max=u_max,
                sigmas=cov_scaler,
                lambda_=args.lambda_,
                env=None,  # Not used with SimplifiedWalker
                mjx_model=mjx_model,  # Passed for compatibility, will use SimplifiedWalker
                gym_env=args.gym_env,
                use_mujoco=True  # This triggers use of kinematics (now redirected to SimplifiedWalker)
            )
        ...
        if args.rgcl:
            ...
            if args.PPO:
                ...
                # Action statistics
                print(f"\n  Action statistics:")
                print(f"    Mean action: {jnp.mean(actions_arr):.4f}")
                print(f"    Std action: {jnp.std(actions_arr):.4f}")
                print(f"    Max action: {jnp.max(jnp.abs(actions_arr)):.4f}")
        ...
        if not args.rgcl and not args.online:
            ...
            for _ in range(REWARD_FUNCTION_UPDATE):
                ...
                # Compute costs for debugging
                if _ == 0:  # Only print on first update
                    agent_costs = jax.vmap(lambda s: state_train.apply_fn({'params':state_train.params}, s.reshape(1,-1)).ravel())(states)
                    expert_costs = jax.vmap(lambda s: state_train.apply_fn({'params':state_train.params}, s.reshape(1,-1)).ravel())(states_expert)
                    print(f"    Agent costs: mean={jnp.mean(agent_costs):.4f}, std={jnp.std(agent_costs):.4f}")
                    print(f"    Expert costs: mean={jnp.mean(expert_costs):.4f}, std={jnp.std(expert_costs):.4f}")

                state_train = update_model(state_train, grads)

                # IMPORTANT: Update policy's state_train so it uses the updated cost function
            ...
            print(f"    Mean loss: {np.mean(loss_rew):.6f}")
            print(f"    Final loss: {loss_rew[-1]:.6f}")

            # Check if cost function parameters are changing
            if i > 0:
                print(f"    Loss change from prev iteration: {np.mean(loss_rew) - mean_loss_rew[-2]:.6f}")
