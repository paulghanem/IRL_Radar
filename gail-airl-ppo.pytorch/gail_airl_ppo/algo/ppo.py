...
class PPOExpert(PPO):
    def __init__(self, state_shape, action_shape, device, path,
                 units_actor=(64, 64)):
        # We intentionally don't call super().__init__ since we don't need buffers,
        # critics, or optimizers — we only need a frozen actor as an expert.
        self.actor = StateIndependentPolicy(
            state_shape=state_shape,
            action_shape=action_shape,
            hidden_units=units_actor,
    ...
    @torch.no_grad()
    def act(self, state):
        """Deterministic action (mean). Expect state as np.array or torch tensor."""
        if not torch.is_tensor(state):
            state = torch.tensor(state, dtype=torch.float32, device=self.device).unsqueeze(0)
        action = self.actor(state, deterministic=True)[0]  # assumes policy supports deterministic flag
        return action.detach().cpu().numpy()

    @torch.no_grad()
    def explore(self, state):
        """Stochastic action and log-prob, to match PPO interface if needed."""
        if not torch.is_tensor(state):
            state = torch.tensor(state, dtype=torch.float32, device=self.device).unsqueeze(0)
        action, log_pi = self.actor.sample(state)
        return action[0].detach().cpu().numpy(), log_pi[0].detach().cpu().numpy()