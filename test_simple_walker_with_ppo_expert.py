#!/usr/bin/env python3
"""
Test Simplified Walker2d with REAL PPO Expert
Uses actual expert demonstrations from trained PPO policy
"""
import os
import sys
import argparse
...
def main():
    ...
    print("=" * 80)
    print(f"SIMPLIFIED WALKER2D WITH PPO EXPERT - {args.method.upper()}")
    print("=" * 80)
    ...
    # Load REAL PPO expert demonstrations
    print("\nLoading PPO expert demonstrations...")
    expert_file = f"expert_demo_Walker2d_seed{args.seed}_steps{args.N_steps}.npz"
    if not os.path.exists(expert_file):
        print(f"ERROR: Expert file not found: {expert_file}")
        print("Please run: python generate_walker2d_expert.py --N_steps 1000 --seed 123")
        sys.exit(1)

    expert_data = np.load(expert_file)
    expert_states = expert_data['states']
    expert_actions = expert_data['actions']
    expert_rewards_cumsum = expert_data['rewards']

    print(f"Loaded expert data:")
    print(f"  States shape: {expert_states.shape}")
    print(f"  Actions shape: {expert_actions.shape}")
    print(f"  Total expert reward: {expert_rewards_cumsum[-1]:.2f}")
    print(f"  Average reward per step: {expert_rewards_cumsum[-1] / args.N_steps:.4f}")

    # Create D_demo in the format expected by MPPI
    D_demo = np.column_stack([expert_states, expert_actions])
    ...
    print("\n" + "=" * 80)
    print(f"RUNNING {args.method.upper()} WITH PPO EXPERT AND SIMPLIFIED WALKER2D")
    print("=" * 80)
    ...
    final_reward = sum(iteration_rewards[-100:]) / 100

    print("\n" + "=" * 80)
    print(f"RESULTS - {args.method.upper()} WITH PPO EXPERT")
    print("=" * 80)
    print(f"Expert reward: {expert_rewards_cumsum[-1]:.2f}")
    ...
    # Save results
    results_file = f"simple_walker2d_ppo_{args.method}_seed{args.seed}_results.txt"
    with open(results_file, 'w') as f:
        f.write(f"Method: {args.method.upper()}\n")
        f.write(f"Expert reward: {expert_rewards_cumsum[-1]:.2f}\n")
        ...
        f.write(f"Average reward (all): {avg_reward:.2f}\n")
        f.write(f"Final reward (last 100): {final_reward:.2f}\n")
        f.write("\nIteration data:\n")
