...
from src.control.dynamics import cartpole_step,get_step_model,get_action_space,kinematics,get_cov_coef
...
class P_MPPI:
    ...
    def generate_session(self, args,epochs,state_train,D_demo,mpc_method=None,thetas=None):
        ...
        cov_timestep = jnp.eye(args.a_dim)*get_cov_coef(args.gym_env)
        ...
        U_nominal = jnp.zeros((args.horizon,args.a_dim))#U_MPPI_init.mean(axis=0)
        ...
        for step in range(1,args.N_steps+1):
            ...
            (U_nominal,cov_prime), (states_nominal, states_MPPI), cost_MPPI, self.key = MPPI_control(state, U_nominal, cov, key,
            ...
            pbar.set_description(f"State = {state} , Action = {U_nominal[-1]} , Cost True = {true_cost_fn(state):.4f} ,  Cost Estimated = {state_train.apply_fn({'params':state_train.params},state.reshape(1,-1)).ravel().item():.8f}")
            pbar.update(1)
