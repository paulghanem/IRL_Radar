...
parser.add_argument("--N_steps",default=1000000,type=int,help="The number of steps in the experiment in GYM ENV")
parser.add_argument("--evaluation_interval",default=10000,type=int,help="evaluation interval")
parser.add_argument("--rollout_length",default=2048,type=int,help="evaluation interval")
...
policy= PPOPolicy(state_dim=args.s_dim,action_dim=args.a_dim, mjx_model=mjx_model,dynamics=get_step_model(args.gym_env,env),policy_model=state_train_p,policy_net=model_p,value_fn=state_train_c,args=args)
...
while step < args.N_steps:
    ...
    states, actions, rewards, dones, log_probs_old, next_states = \
        policy.buffer.get()
    key = jax.random.PRNGKey(args.seed)
    #states, actions, rewards, dones, log_probs_old, next_states = \
     #   policy.buffer.sample(key=key,batch_size=args.rollout_length)
