#!/bin/bash

# Submit only jobs that are NOT currently running, using L40S GPUs

SEEDS=(123 124 125 126)
ENVS=("Walker2d" "Hopper" "HalfCheetah-v4" "Swimmer")
METHODS=("--rgcl" "--UB" "--airl" "--gail" "--sqil" "")

mkdir -p slurm_logs

# Get list of currently running jobs
squeue --me -o "%.50j %.2t" | grep "R$" | awk '{print $1}' > /tmp/running_jobs.txt

echo "==========================================="
echo "Submitting PENDING jobs only (V100-16GB)"
echo "==========================================="
echo ""

submitted_count=0
skipped_count=0

for env in "${ENVS[@]}"; do
    for seed in "${SEEDS[@]}"; do
        for method in "${METHODS[@]}"; do

            if [ -z "$method" ]; then
                METHOD_NAME="baseline"
                METHOD_FLAG=""
            else
                METHOD_NAME="${method//-/}"
                METHOD_FLAG="$method"
            fi

            JOB_NAME="${env}_${METHOD_NAME}_${seed}"

            # Check if job is currently running
            if grep -q "^${JOB_NAME}$" /tmp/running_jobs.txt; then
                echo "SKIPPING (already running): $JOB_NAME"
                ((skipped_count++))
                continue
            fi

            # Set Q value based on method
            if [ "$method" == "--rgcl" ]; then
                Q_VALUE="1e-6"
            else
                Q_VALUE="1e-5"
            fi

            echo "Submitting: $JOB_NAME (V100-16GB, Q=${Q_VALUE})"

            sbatch <<EOT
#!/bin/bash
#SBATCH --job-name=${JOB_NAME}
#SBATCH --output=slurm_logs/${JOB_NAME}.out
#SBATCH --error=slurm_logs/${JOB_NAME}.err
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=1
#SBATCH --cpus-per-task=4
#SBATCH --gpus=v100-16:1
#SBATCH --time=48:00:00
#SBATCH --partition=GPU-shared
#SBATCH --mail-user=ghanem.p@northeastern.edu
#SBATCH --mail-type=END,FAIL

module load anaconda3/2024.10-1
module load cuda/12.4.0
conda activate rirl

echo "Running ${JOB_NAME} on \$(hostname)"
echo "GPU Allocated: \$CUDA_VISIBLE_DEVICES"

python main.py \
    --gym_env="${env}" \
    --num_traj=500 \
    --horizon=20 \
    --N_steps=1000 \
    --N_steps_expert=1000 \
    --rirl_iterations=1000 \
    --reward_fn_updates=15 \
    ${METHOD_FLAG} \
    --seed=${seed} \
    --lr=1e-4 \
    --lambda_=0.01 \
    --Q=${Q_VALUE} \
    --P=1e-2 \
    --hidden_dim=16 \
    --no-save_images

echo "Job ${JOB_NAME} complete"
EOT

            ((submitted_count++))
        done
    done
done

echo ""
echo "==========================================="
echo "Submission Summary:"
echo "  Jobs submitted: $submitted_count"
echo "  Jobs skipped (running): $skipped_count"
echo "  GPU: V100-16GB"
echo "==========================================="
