...
def cart_pole_cost(state,state_train=None,goal_x = 0.0,goal_theta = 0.0):
    x = state[...,0]
    theta = state[...,2]
...
def pendudulum_cost(state,state_train=None):
...
def mountaincar_cost(state,state_train=None):
    position = state[...,0]
    velocity = state[...,1]
    goal_position = 0.45
    goal_velocity = 0.0
    ...
    # )
    cost = 10 * (position - goal_position)**2 # + 0.1 * velocity**2
    return cost
